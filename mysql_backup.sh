...
/usr/bin/ssh  -x -l backuppc $SERVER &&
sudo docker run -it --rm --link $CONTAINER:$CONTAINER &&
...
if [ -s $DAILYLOGFILE ]; then    #email only if error ocured - ie log file exists
    cat "$DAILYLOGFILE"  | /usr/bin/mutt -s "Servers Backup Log" $EMAIL
    if [[ $? -ne 0 ]]; then
        echo -e "ERROR:  Error log  could not be emailed to you! \n";
        else
        echo -e "Backup Error log has been emailed to the admin! \n"
    fi
fi
